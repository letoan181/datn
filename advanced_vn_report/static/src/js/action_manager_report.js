odoo.define('advanced_account_reports.ActionManager', function (require) {
"use strict";
...
ActionManager.include({
     ...
     */
    _executeAccountReportTemplateDownloadAction: function (action) {
        var self = this;
        framework.blockUI();
        return new Promise(function (resolve, reject) {
            session.get_file({
                url: '/advanced/account_reports',
                data: action.data,
                success: resolve,
                error: (error) => {
    ...
    _handleAction: function (action, options) {
        if (action.type === 'ir_actions_account_report_template_download') {
            return this._executeAccountReportTemplateDownloadAction(action, options);
        }
