...
class GoogleHangoutHelper:
    ...
    def drop_file_permission_callback(self, request_id, response, exception):
        if exception:
            raise UserError(exception.content)
        else:
    ...
    def detail_permission(self, file_id, permission_id):
        permission = self._get_driver_service().permissions().get(
            fileId=file_id, permissionId=permission_id, fields='kind,id,type,emailAddress,domain,role,allowFileDiscovery,displayName,photoLink,expirationTime,teamDrivePermissionDetails,deleted').execute()
