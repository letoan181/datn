...
class SnippetContactUs(http.Controller):
    ...
    def website_form(self, model_name, **kwargs):
        ...
        if model_name == 'crm.lead' and not request.params.get('state_id'):
            ...
            if geoip_country_code and geoip_state_code:
                ...
                request.params['state_id'] = State.search(
                    [('code', '=', geoip_state_code), ('country_id.code', '=', geoip_country_code)]).id
        # Check model
        ...
        try:
            ...
            # template = request.env.ref('advanced_contact_form.new_lead_coming_template')
            # current_record = request.env['crm.lead'].sudo().browse(id_record)
            # if len(current_record.message_partner_ids.ids) > 0:
            #     try:
            #         request.env['mail.template'].sudo().browse(template.id) \
            #             .send_mail(id_record, force_send=True,
            #                        email_values={
            #                            'recipient_ids': [(4, e) for e in current_record.message_partner_ids.ids]})
            #     except Exception as ex:
            #         a = 0
            # send auto email respond to customer
            template = request.env.ref('advanced_crm.lead_auto_responder_mail')
            current_record = request.env['crm.lead'].sudo().browse(id_record)
            if current_record.email_from:
                try:
                    request.env['mail.template'].sudo().browse(template.id) \
                        .send_mail(id_record, force_send=True,
                                   email_values={
                                       'email_to': current_record.email_from})
                except Exception as ex:
                    a = 0
        ...
        return request.redirect(kwargs['success_page'])
    ...
    def extract_data(self, model, values):
        ...
        if 'source_detail' in values:
            data['record'].update({
                'source_detail': values['source_detail'],
            })
        if 'lead_location' in values:
            data['record'].update({
                'lead_location': int(values['lead_location']),
            })
        if 'project_type' in values:
            data['record'].update({
                'project_type': int(values['project_type']),
            })
        if 'lead_type' in values:
            data['record'].update({
